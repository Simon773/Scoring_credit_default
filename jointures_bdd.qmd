...
```{r}
dat_default_moodys_annual <- readRDS("data/dat_default_moodys_annual.rds")
```
...
On enlève les industries de rem_ind

```{r}
rem_ind <- c("Agriculture", "Finance", "Mining", "Public")
df <- join_cclm %>% filter(! industry %in% rem_ind)
nrow(df) != nrow(join_cclm) #doit être TRUE
```

```{r}
saveRDS(df, "data/df_joined.rds")
```
