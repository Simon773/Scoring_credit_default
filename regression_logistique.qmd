...
data %>% select(default3, WCTA, RETA, EBTA, TLTA, SLTA) %>% summary()
```
...
data_altman <- data %>% drop_na(WCTA, RETA, EBTA, TLTA, SLTA)
data_altman %>% select(default3, WCTA, RETA, EBTA, TLTA, SLTA) %>% summary()
```

Revoir le traitement des variables manquantes

```{r}
altman_model <- glm(default3 ~ WCTA + RETA + EBTA + TLTA + SLTA, family="binomial", data = data_altman)
...
freq <- table(data_altman$default3)
data_altman$weights <- 1 / freq[match(data_altman$default3, names(freq))]

#Model
altman_model_weights <- glm(default3 ~ WCTA + RETA + EBTA + TLTA + SLTA, family="binomial", weights = weights, data = data_altman)
...
```{r}
# Calculer proportion
prop <- prop.table(table(data_altman$default3))
data_altman <- data_altman %>%
  mutate(weights_2 = 1 / prop[match(default3, names(prop))])
...
```{r}
altman_model_weights_prop <- glm(default3 ~ WCTA + RETA + EBTA + TLTA + SLTA, family="binomial", weights = weights_prop, data = data_altman)
...
data_altman_win %>% select(default3, WCTA_win, RETA_win, EBTA_win, TLTA_win, SLTA_win) %>% summary()
```

```{r}
# Calculer proportion
prop <- prop.table(table(data_altman_win$default3))
data_altman_win <- data_altman_win %>%
  mutate(weights_2 = 1 / prop[match(default3, names(prop))])

# Normaliser pour que la moyenne des poids = 1
data_altman_win$weights_prop <- data_altman_win$weights_2 / mean(data_altman_win$weights_2)
```

```{r}
altman_model_weights_prop <- glm(default3 ~ WCTA_win + RETA_win + EBTA_win + TLTA_win + SLTA_win, family="binomial", weights = weights_prop, data = data_altman_win)
summary(altman_model_weights_prop)
```
